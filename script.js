...
async function loadPools() {
  try {
    ...
    alert('Error loading pool list. See console for details.');
    pools = [];
  }
}
...
/* MAP INITIALISATION — now done after pools load */
...
async function init() {
  ...
  if (!pools.length) {
    console.error('No pools loaded — check pools.json path.');
    renderList();
    return;
  }

  // make sure selectedIndex is in range
  ...
  setTimeout(() => { if (map) { map.invalidateSize(); panToSelected(); } }, 150);
  setView(false);
  updateCount();
}
